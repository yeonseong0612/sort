...
class NeighborImputation:
    def __init__(self, k=7, alpha=0.5):
        self.k = k
        self.alpha = alpha  # fallback; superseded by gating when gating is provided
    ...
    def impute(self, target_track, active_tracks, gating: GatingNetwork = None):
        """
        x_imputed = x_kalman + alpha * (v_neighbor - v_self)
        이웃이 없으면 칼만 예측값(x_kalman)으로 폴백.
        gating이 주어지면 w_neighbor를 alpha로 사용; 없으면 self.alpha 사용.
        ...
        if gating is not None:
            _, alpha = gating.get_weights(target_track, active_tracks)
        else:
            alpha = self.alpha
        ...
        x_imputed = target_track.mean[:2] + alpha * (v_neighbor - v_self)
